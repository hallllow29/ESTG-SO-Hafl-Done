...
public class Mem {
    ...
    public synchronized boolean allocateFF(Task task) {
        if (!isRunning) {
            logger.warning("MEMORY DID NOT STARTED YET");
            return false;
        }

        if (task.getMemorySize() <= 0) {
            logger.warning("TASK MEMORY SIZE CANNOT BE ZERO");
        }

        if (this.totalMemoryUsed + task.getMemorySize() > MAX_MEMORY_SIZE) {
            logger.warning("TASK MEMORY SIZE SUPERSEEDS MAX MEMORY SIZE");
            return false;
        }
        ...
        for (MemoryBlock block : this.memoryFree) {
            ...
            if (block.getSize() >= task.getMemorySize()) {
                ...
                this.memoryBlocks.put(newBlock.getId(), newBlock);
                this.totalMemoryUsed += newBlock.getSize();
				...
				logger.info("Memory allocated for " + newBlock.getId() + " with size " + newBlock.getSize());
                printMemoryStatus();
                return true;
            }
        }

		logger.warning("Failed allocation for Core.MemoryBlock " + task.getName() + " with size " + task.getMemorySize());
		return false;
    }
...
/* public synchronized boolean allocateBF(String id, int requestedSize) {
        ...
        Core.MemoryBlock bestBlock = null;

        for (Core.MemoryBlock block : this.memoryFree) {
        ...
        if (bestBlock != null) {
            ...
            Core.MemoryBlock newBlock = new Core.MemoryBlock(id, startAddress, requestedSize);
		...
		logger.warning("Failed allocation for Core.MemoryBlock " + id + " with size " + requestedSize);
		return false;
    }*/
    ...
    /*public synchronized boolean allocateWF(String id, int requestedSize) {
        ...
        Core.MemoryBlock worstBlock = null;
    ...
    /* private void mergeMemoryFreeBlocks() throws EmptyCollectionException, ElementNotFoundException {
        ...
        LinearNode<Core.MemoryBlock> currentNode = this.memoryFree.getFront();

        while (currentNode != null && currentNode.getNext() != null) {
            Core.MemoryBlock currentBlock = currentNode.getElement();
            Core.MemoryBlock nextBlock = currentNode.getNext().getElement();
